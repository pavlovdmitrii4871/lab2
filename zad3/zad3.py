...
def resh(n, source, target, auxiliary):
    if n == 1:
        disk = source.pop()
        target.push(disk)  # Переносим диск с одного стека на другой
        return

    resh(n-1, source, auxiliary, target)  # Рекурсивный вызов функции с переносом n-1 диска на промежуточный стек
    disk = source.pop()
    target.push(disk)
    resh(n-1, auxiliary, target, source)  # Рекурсивный вызов функции с переносом n-1 диска с промежуточного стека на конечный
...
# Вызов функции resh для решения задачи
resh(stackl_a.size(), stackl_a, stackl_c, stackl_b)
